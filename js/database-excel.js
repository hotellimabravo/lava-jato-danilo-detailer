...
class DatabaseExcelService {
	...
	static exportarBaseCompleta() {
		...
		const dadosNegocio = [
			...
			{ 'Campo': 'Tipo de Negócio', 'Valor': configNegocio.tipoNegocio || 'geral' },
		...
		const dadosMeta = [
			{ 'Propriedade': 'Sistema', 'Valor': configNegocio.nomeEstabelecimento ? `${(configNegocio.tipoNegocioCustom || configNegocio.tipoNegocio).toUpperCase()} - ${configNegocio.nomeEstabelecimento}` : 'OUTRO SEGMENTO - SEU NEGÓCIO' },
		...
		// Gerar objeto Workbook com todas as abas
		const wb = this.gerarWorkbookCompleto();
	...
	// Gera a estrutura do Workbook em memória (usado para exportação e sincronização no Drive)
	static gerarWorkbookCompleto() {
		if (typeof XLSX === 'undefined') {
			throw new Error('A biblioteca SheetJS/XLSX não foi carregada.');
		}
		...
		const dadosMeta = [
			...
			{ 'Propriedade': 'Total de Ordens de Serviço', 'Valor': pedidos.length },
			{ 'Propriedade': 'Total de Fechamentos de Caixa', 'Valor': caixas.length },
			{ 'Propriedade': 'Formato', 'Valor': 'Database Google Drive / Excel (.xlsx)' }
		];
	...
	// Converte o Workbook atual em Uint8Array / ArrayBuffer para upload
	static gerarArrayBufferCompleto() {
		const wb = this.gerarWorkbookCompleto();
	...
	static baixarModeloExemplo() {
		...
		const modeloNegocio = [
			{ 'Campo': 'Nome do Estabelecimento', 'Valor': 'SEU NEGÓCIO' },
			{ 'Campo': 'Razão Social', 'Valor': 'Razão Social da Sua Empresa' },
			{ 'Campo': 'CNPJ', 'Valor': '00.000.000/0001-00' },
			{ 'Campo': 'Tipo de Negócio', 'Valor': 'geral' },
			{ 'Campo': 'Tipo de Negócio Personalizado', 'Valor': 'OUTRO SEGMENTO' },
			{ 'Campo': 'Telefone / WhatsApp', 'Valor': '(11) 99999-0000' },
			{ 'Campo': 'Endereço Completo', 'Valor': 'Av. Principal, 1200 - Centro' },
			{ 'Campo': 'Responsável pelo Estabelecimento', 'Valor': 'Responsável' },
			{ 'Campo': 'Ícone do Logo', 'Valor': '🛠️' }
		];
	...
	// ----------------------------------------------------------------------
	// Processar e Importar diretamente a partir de um ArrayBuffer / Uint8Array (ex: Google Drive)
	// ----------------------------------------------------------------------
	static processarArrayBuffer(arrayBuffer, nomeOrigem = 'Google Drive', modo = 'substituir') {
		if (typeof XLSX === 'undefined') {
			throw new Error('Biblioteca XLSX não carregada no navegador.');
		}
		...
		if (sheetClientesNome) {
			...
			novosClientes = json.map(r => ({
				nome: String(r['Nome do Cliente'] || r['Nome'] || r['nome'] || '').trim(),
				tel1: String(r['Telefone (Código ID)'] || r['Telefone 1'] || r['tel1'] || '').trim(),
				wpp1: String(r['WhatsApp Principal'] || r['wpp1'] || '').includes('✅') || String(r['WhatsApp Principal'] || '').toLowerCase() === 'sim',
				tel2: String(r['Telefone Secundário'] || r['Telefone 2'] || r['tel2'] || '').trim(),
				wpp2: String(r['WhatsApp Secundário'] || r['wpp2'] || '').includes('✅') || String(r['WhatsApp Secundário'] || '').toLowerCase() === 'sim',
				endereco: String(r['Endereço'] || r['Endereco'] || r['endereco'] || '').trim()
		...
		// 5. Processar Configuração do Negócio
		const sheetNegocioNome = workbook.SheetNames.find(n => 
			n.toLowerCase().includes('negocio') || n.toLowerCase().includes('negócio') || n.toLowerCase().includes('empresa') || n.toLowerCase().includes('estabelecimento')
		);
		if (sheetNegocioNome) {
